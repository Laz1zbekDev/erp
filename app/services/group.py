...
from sqlalchemy import select, func, delete, update, String, literal
...
async def create_group(session: AsyncSession, data: CreateGroup) -> Group:
    group = Group(
        ...
        science_name=data.science_name.strip().capitalize(),
        name=data.name,
        price=data.price,
...
async def get_all_science(session: AsyncSession) -> list[Group]:
    stmt = (
        select(Group)
        .where(Group.status)
        .distinct(func.lower(Group.science_name))
        .order_by(func.lower(Group.science_name))
    )
...
async def search_groups(
    ...
    teacher_id: int | None,
    group_day: str | None,
    group_date: time | None,
    science_name: str | None,
) -> list[Group]:
    stmt = select(Group).where(Group.status).options(selectinload(Group.teacher))

    if group_name:
        stmt = stmt.where(Group.name.ilike(f"%{group_name.strip()}%"))
    if teacher_id:
        stmt = stmt.where(Group.teacher_id == teacher_id)
    if group_day:
        stmt = stmt.where(
            literal(group_day.lower())
            == func.any(
    ...
    if group_date:
        stmt = stmt.where(Group.class_date == group_date)
    if science_name:
        stmt = stmt.where(Group.science_name.ilike(f"%{science_name.strip()}%"))
...
async def get_students_by_group(session: AsyncSession, group_id: int):
    result = await session.execute(
        ...
        .options(
            ...
            selectinload(Group.student_per_dates),
...
async def get_group_students(session: AsyncSession, group_id: int) -> Group | None:
    result = await session.execute(
        select(Group)
        .where(Group.group_id == group_id)
        .options(
...
async def get_groups(session: AsyncSession) -> list[Group]:
    ...
    result = await session.scalars(
        select(Group).where(Group.status).options(selectinload(Group.teacher))
    )
...
async def update_group(
    session: AsyncSession, group_id: int, data: UpdateGroup
) -> Group:
    ...
    group.name = data.name.strip().capitalize() if data.name else group.name
    group.science_name = (
        data.science_name.strip().capitalize()
        if data.science_name
        else group.science_name
    )
    ...
    group.class_day = (
        data.class_day.strip().capitalize() if data.class_day else group.class_day
    )
    group.teacher_percent = (
        data.teacher_percent if data.teacher_percent else group.teacher_percent
    )

    return group


async def update_group_teacher(
...
async def delete_group(session: AsyncSession, group_id: int) -> None:
    ...
    await session.execute(
        delete(StudentPermissionDate).where(StudentPermissionDate.group_id == group_id)
    )
    await session.execute(
        update(StudentDiscount)
        .where(StudentDiscount.group_id == group_id)
        .values(discount_amount=0)
    )
    await session.execute(
        update(StudentGr).where(StudentGr.group_id == group_id).values(status=False)
    )
    group.status = False
    group.name = f"{group.created_at}{group.name}{datetime.now()}"
